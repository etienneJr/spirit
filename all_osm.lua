...
-- which stores tags (as jsonb) and geometries
-- in 1 table per OSM type : nodes_geom, ways_geom, rels_geom

-- use with command :
-- osm2pgsql -d spirit -c -s -O flex -S geometries-alone.lua data/extract.osm.pbf

-- For Europe or planet, add --flat-nodes DB-flat-nodes --middle-with-nodes

-- then create indexes for tags with
-- CREATE INDEX nodes_tags_idx ON nodes_geom USING GIN (tags);
-- CREATE INDEX ways_tags_idx ON ways_geom USING GIN (tags);
-- CREATE INDEX rels_tags_idx ON rels_geom USING GIN (tags);
